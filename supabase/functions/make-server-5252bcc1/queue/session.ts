/**
 * Quecumber — Session Lifecycle Management
 */
 ...
 */
async function indexSessionForLocation(
  locationId: string,
  sessionId: string,
): Promise<void> {
  const existing: string[] =
    (await kv.get(`location_sessions:${locationId}`)) || [];
...
export async function getActiveSession(
  ...
  const queueTypes = await getQueueTypesForLocation(
    locationId,
  ...
  for (const locId of locationIds) {
    ...
    if (!hoursCheck.isOpen) {
      const result = await closeAllSessionsForLocation(
        locId,
        "Auto-close: past business hours",
      );
...
export async function midnightRotation(
  ...
  await archiveOldSessions(locationId, 30).catch((err) => {
    console.log(
      `[midnightRotation] Archive warning for ${locationId}: ${err.message}`,
    );
  });
