...
export function register(app: Hono) {
    app.post("/onboarding/business", async (c: any) => {
        try {
            ...
            if (!user) return c.json({ error: "Unauthorized while creating business" }, 401);
            ...
            const business = {
                ...
                industry: body.industry || null, phone: body.phone || null,
                email: body.email || user.email, address: body.address || null,
                owner_id: user.id, status: "active", created_at: timestamp, updated_at: timestamp,
            };
            await kv.set(`business:${businessId}`, business);
            const staffUser = {
                id: uuid(), auth_user_id: user.id, business_id: businessId,
                email: user.email, name: user.user_metadata?.name || body.ownerName || "",
                role: "owner", status: "active", locations: [], created_at: timestamp, updated_at: timestamp,
            };
    ...
    app.post("/onboarding/location", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized while creating location" }, 401);
            const body = await c.req.json();
            const locationId = uuid();
            const timestamp = now();
            const slug = (body.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
            const location = {
                id: locationId, business_id: body.businessId, name: body.name, slug,
                address: body.address || null, city: body.city || null, phone: body.phone || null,
                country: body.country || null, status: "active", timezone: body.timezone || "Europe/Istanbul",
    ...
    app.post("/onboarding/queue-types", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized while creating queue types" }, 401);
            const body = await c.req.json();
            ...
            for (const qt of queueTypes) {
                ...
                const queueType = {
                    id: queueTypeId, business_id: businessId, location_id: locationId, name: qt.name,
                    prefix: qt.prefix || qt.name.charAt(0).toUpperCase(), description: qt.description || null,
                    estimated_service_time: qt.estimatedServiceTime || 10, max_capacity: qt.maxCapacity || 100,
                    status: "active", sort_order: qt.sortOrder || created.length, created_at: timestamp, updated_at: timestamp,
                };
    ...
    app.post("/onboarding/business-hours", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized while saving business hours" }, 401);
            const body = await c.req.json();
            await kv.set(`business_hours:${body.locationId}`, {
                business_id: body.businessId, location_id: body.locationId, hours: body.hours, updated_at: now(),
            });
    ...
    app.post("/onboarding/whatsapp", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized while saving WhatsApp settings" }, 401);
            const body = await c.req.json();
            ...
            const settings = {
                business_id: body.businessId, enabled: !!body.enabled,
                phone_number: normalizePhoneNumber(body.phoneNumber), updated_at: now(),
            };
            ...
            whatsapp.sendWelcomeMessage({
                businessId: body.businessId, phone: welcomePhone, locale: (body.locale as any) || "en",
                customerName: staffRecord?.name || "Owner", businessName: business?.name || "Your Business",
            ...
            return c.json({ error: `WhatsApp settings save failed: ${err.message}` }, 500);
        }
    });
        ...
        try {
            ...
            for (const member of staffMembers) {
                const { data: authData, error: authError } = await supabase.auth.admin.createUser({
                    email: member.email, password: member.password || "EMFlow2026!",
                    user_metadata: { name: member.name }, email_confirm: true,
                });
                if (authError) { console.log(`Staff creation warning for ${member.email}: ${authError.message}`); continue; }
                const staffUser = {
                    id: uuid(), auth_user_id: authData.user.id, business_id: businessId,
                    email: member.email, name: member.name, role: member.role || "staff",
                    status: "active", locations: [locationId], created_at: timestamp, updated_at: timestamp,
                };
    ...
    app.post("/onboarding/complete", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized while completing onboarding" }, 401);
            const staffRecord = await kv.get(`staff_user:${user.id}`);
            if (staffRecord) {
            ...
            return c.json({ error: `Onboarding completion failed: ${err.message}` }, 500);
        }
    });
}
