...
export function register(app: Hono) {
  ...
  app.post("/cron/auto-close-sessions", async (c: any) => {
    try {
      ...
      console.log(
        `[cron/auto-close] Business ${businessId}: processed ${result.locationsProcessed} locations, closed ${result.totalClosed} sessions, cancelled ${result.totalCancelled} entries`,
      );
      return c.json(result);
    } catch (err: any) {
  ...
  app.post("/cron/midnight-rotation/:locationId", async (c: any) => {
    try {
      ...
      console.log(
        `[cron/midnight] Location ${locationId}: closed ${result.closedPrevious} sessions, cancelled ${result.cancelledEntries} entries`,
      );
      return c.json(result);
    } catch (err: any) {
