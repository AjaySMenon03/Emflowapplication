/**
 * EM Flow — Realtime Polling Route
 */
...
export function register(app: Hono) {
    app.get("/realtime/poll/:locationId", async (c: any) => {
        try {
            ...
            const currentCounter = (await kv.get(`realtime_counter:${locationId}`)) || 0;
            if (currentCounter > since) {
                const latestEvent = await kv.get(`realtime_event:${locationId}`);
                return c.json({ hasChanges: true, counter: currentCounter, event: latestEvent });
            }
