/**
 * Quecumber — Settings Routes (Queue Types, Services, Staff, Business, Location, WhatsApp, Business Hours)
 */
  ...
  app.get("/settings/services/:businessId", async (c: any) => {
    try {
      ...
      const serviceIds: string[] =
        (await kv.get(`business_services:${businessId}`)) || [];
      const services: any[] = [];
  ...
  app.post("/settings/services", async (c: any) => {
    try {
      ...
      const existing: string[] =
        (await kv.get(`business_services:${staffRecord.business_id}`)) || [];
  ...
  app.put("/settings/staff/:authUid", async (c: any) => {
    try {
      ...
      if (!staffRecord || !["owner", "admin"].includes(staffRecord.role))
        return c.json(
          { error: "Only owners and admins can manage staff" },
          403,
        );
      ...
      if (staffRecord.role === "admin" && target.role !== "staff")
        return c.json(
          { error: "Admins can only edit staff-role members" },
          403,
        );
      const body = await c.req.json();
      const newRole =
