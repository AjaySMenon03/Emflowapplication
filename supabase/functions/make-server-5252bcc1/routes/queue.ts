...
export function register(app: Hono) {
    app.get("/queue/entries/:locationId", async (c: any) => {
        try {
            ...
            const waitingEnriched = await Promise.all(waitingRaw.map(async (e: any, idx: number) => {
                const eta = await queueLogic.calculateETA(e.id);
                return { ...e, estimatedMinutes: eta.estimatedMinutes };
            }));
            const waiting = waitingEnriched.sort((a: any, b: any) => {
            ...
            const completed = entries.filter((e: any) => e.status === "served" || e.status === "no_show" || e.status === "cancelled")
                .sort((a: any, b: any) => new Date(b.completed_at || b.cancelled_at || b.created_at).getTime() - new Date(a.completed_at || a.cancelled_at || a.created_at).getTime())
                .slice(0, 50);
    ...
    app.post("/queue/call-next", async (c: any) => {
        try {
            ...
            const entry = await queueLogic.callNext({ queueTypeId, sessionId, staffAuthUid: user.id });
            ...
            if (entry.customer_phone) {
                ...
                whatsapp.sendYourTurnNotification({
                    ...
                    ticketNumber: entry.ticket_number, queueName: entry.queue_type_name || "Queue", businessName: business?.name,
                }).catch((err: any) => console.log(`[WhatsApp call-next] Error: ${err.message}`));
            }
            notifyPosition3ViaWhatsApp(entry.queue_type_id, entry.queue_session_id).catch((err: any) => console.error(`[Position3 notify] Error: ${err.message}`));
            return c.json({ entry });
        } catch (err: any) {
    ...
    app.post("/queue/start-serving/:entryId", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const entry = await queueLogic.startServing(c.req.param("entryId"), user.id);
            return c.json({ entry });
        } catch (err: any) {
    ...
    app.post("/queue/reassign/:entryId", async (c: any) => {
        try {
            ...
            const entry = await queueLogic.reassignStaff(c.req.param("entryId"), newStaffAuthUid);
            return c.json({ entry });
        } catch (err: any) {
    ...
    app.post("/queue/session-smart", async (c: any) => {
        try {
            const user = await getAuthUser(c);
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const { queueTypeId, locationId, businessId, skipBusinessHoursCheck } = await c.req.json();
            const result = await queueLogic.getOrCreateTodaySessionSmart(queueTypeId, locationId, businessId, { skipBusinessHoursCheck: !!skipBusinessHoursCheck });
    ...
    app.post("/queue/cancel-enhanced/:entryId", async (c: any) => {
        try {
            ...
            return c.json({ error: `Cancel failed: ${err.message}` }, err.message.includes("Cannot cancel") || err.message.includes("already") ? 400 : 500);
        }
    });
    ...
    app.post("/queue/mark-previous-served", async (c: any) => {
        try {
            ...
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const { queueTypeId, sessionId } = await c.req.json();
            const entry = await queueLogic.markPreviousAsServed({ queueTypeId, sessionId, staffAuthUid: user.id });
            if (!entry) return c.json({ entry: null, message: "No previous entry to mark" });
            return c.json({ entry });
        } catch (err: any) {
    ...
    app.post("/queue/session/close/:sessionId", async (c: any) => {
        try {
            ...
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const staffRecord = await kv.get(`staff_user:${user.id}`);
            if (!staffRecord || (staffRecord.role !== "owner" && staffRecord.role !== "admin")) return c.json({ error: "Only owners/admins can close sessions" }, 403);
    ...
    app.post("/queue/session/close-all/:locationId", async (c: any) => {
        try {
            ...
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const staffRecord = await kv.get(`staff_user:${user.id}`);
            if (!staffRecord || (staffRecord.role !== "owner" && staffRecord.role !== "admin")) return c.json({ error: "Only owners/admins can close all sessions" }, 403);
            const locationId = c.req.param("locationId");
            const body = await c.req.json().catch(() => ({}));
            const result = await queueLogic.closeAllSessionsForLocation(locationId, body.reason || "Manually closed all sessions");
            return c.json(result);
        } catch (err: any) {
            return c.json({ error: `Close all sessions failed: ${err.message}` }, 500);
        }
    });
    ...
    app.post("/queue/session/archive/:locationId", async (c: any) => {
        try {
            ...
            if (!user) return c.json({ error: "Unauthorized" }, 401);
            const staffRecord = await kv.get(`staff_user:${user.id}`);
            if (!staffRecord || staffRecord.role !== "owner") return c.json({ error: "Only owners can archive sessions" }, 403);
            const locationId = c.req.param("locationId");
            const body = await c.req.json().catch(() => ({}));
